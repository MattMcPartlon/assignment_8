# RB tree implementation Modified from : https://algorithmtutor.com/Data-Structures/Tree/Red-Black-Trees/
...
class Node:
    ...
    def __eq__(self, other): #weak check
        if not isinstance(other, Node):
            return False
        return other.id == self.id

    def __ne__(self, other): #weak check
        if not isinstance(other, Node):
            return True
        return other.id != self.id
...
class RedBlackTree:
    ...
    def __delete_node_helper(self, node, key):
        ...
        if z == self.TNULL:
            print("Couldn't find key",key," in the tree")
            #self.pretty_print()
            return
...
if __name__ == "__main__":
    ...
    for key in keys:
        bst.insert(key)
    removed = np.random.choice(keys, size = 20)
    print(removed)
    for key in removed:
        bst.delete(key)
    for key in removed:
